...
#define DOCTEST_SUPPRESS_COMMON_WARNINGS_PUSH                                  \
    ...
    DOCTEST_MSVC_SUPPRESS_WARNING(                                             \
        5045) /* Spectre mitigation for memory load */                         \
    DOCTEST_MSVC_SUPPRESS_WARNING(                                             \
...
#define DOCTEST_MAKE_STD_HEADERS_CLEAN_FROM_WARNINGS_ON_WALL_BEGIN             \
    ...
    DOCTEST_MSVC_SUPPRESS_WARNING(                                             \
        4774) /* format string not a string literal */                         \
    DOCTEST_MSVC_SUPPRESS_WARNING(4820) /* padding */                          \
    DOCTEST_MSVC_SUPPRESS_WARNING(                                             \
        4625) /* copy constructor was implicitly deleted */                    \
    DOCTEST_MSVC_SUPPRESS_WARNING(                                             \
    ...
    DOCTEST_MSVC_SUPPRESS_WARNING(                                             \
        4738) /* storing float result in memory, loss of performance */        \
...
typedef basic_ostream<char, char_traits<char>>
    ostream; // NOLINT(modernize-use-using)
...
typedef basic_istream<char, char_traits<char>>
    istream; // NOLINT(modernize-use-using)
...
class DOCTEST_INTERFACE String
{
  ...
  private:
    static DOCTEST_CONSTEXPR size_type len =
        24; //! OCLINT avoid private static members
...
struct DOCTEST_INTERFACE AssertData
{
    ...
    AssertData(assertType::Enum at, const char *file, int line,
...
struct ContextOptions //! OCLINT too many fields
{
    ...
    bool gnu_file_line; // if line numbers should be surrounded with :x: and not
                        // (x):
    bool no_path_in_filenames;  // if the path to files should be removed from
                                // the output
    String strip_file_prefixes; // remove the longest matching one of these
                                // prefixes from any file paths in the output
...
template <bool C> struct StringMakerBase
{
    template <typename T> static String convert(const DOCTEST_REF_WRAP(T))
    {
        ...
        static_assert(deferred_false<T>::value,
                      "No stringification detected for type T. See string "
                      "conversion manual");
#endif
        return "{?}";
    }
...
template <typename T> String toString()
{
    ...
    String ret = __FUNCSIG__; // class doctest::String __cdecl
                              // doctest::toString<TYPE>(void)
...
#else
    String ret =
        __PRETTY_FUNCTION__; // doctest::String toString() [with T = TYPE]
...
struct DOCTEST_INTERFACE ExpressionDecomposer
{
    ...
    // (based on the operator precedence table) but then there will be warnings
    // from GCC about "-Wparentheses" and since "_Pragma()" is problematic this
    // will stay for now... https://github.com/catchorg/Catch2/issues/870
    // https://github.com/catchorg/Catch2/issues/565
    template <typename L>
    Expression_lhs<const L &&> operator<<(const L &&operand)
    { // bitfields bind to universal ref but not const rvalue ref
        return Expression_lhs<const L &&>(static_cast<const L &&>(operand),
                                          m_at);
    }
...
template <typename Reporter> IReporter *reporterCreator(const ContextOptions &o)
{
...
#define DOCTEST_REGISTER_EXCEPTION_TRANSLATOR_IMPL(translatorName, signature)  \
    ...
    DOCTEST_GLOBAL_NO_WARNINGS(                                                \
        DOCTEST_ANONYMOUS(                                                     \
            DOCTEST_ANON_TRANSLATOR_), /* NOLINT(cert-err58-cpp) */            \
...
#define DOCTEST_REGISTER_REPORTER(name, priority, reporter)                    \
    DOCTEST_GLOBAL_NO_WARNINGS(                                                \
        DOCTEST_ANONYMOUS(                                                     \
            DOCTEST_ANON_REPORTER_), /* NOLINT(cert-err58-cpp) */              \
...
#define DOCTEST_WARN_THROWS_WITH(expr, with, ...)                              \
    []                                                                         \
    {                                                                          \
        static_assert(false, "Exception translation is not available when "    \
                             "doctest is disabled.");                          \
        return false;                                                          \
    }()
...
#define DOCTEST_EXCEPTION_EMPTY_FUNC                                           \
    ...
    {                                                                          \
        static_assert(false,                                                   \
                      ...
                      "Use DOCTEST_CONFIG_NO_EXCEPTIONS_BUT_WITH_ALL_ASSERTS " \
                      "if you want to compile with exceptions disabled.");     \
        return false;                                                          \
    }()
...
template <typename T> class MultiLaneAtomic
{
    ...
    {
        Atomic<T> atomic{};
        char padding[DOCTEST_MULTI_LANE_ATOMICS_CACHE_LINE_SIZE -
                     sizeof(Atomic<T>)];
    };
  ...
  private:
    ...
    //
    // The logic here is a bit tricky. The call should be as fast as possible,
    // so that there is minimal to no overhead in determining the correct atomic
...
{
    if (isOnStack())
        return last - (size_type(buf[last]) &
...
const char *skipPathFromFilename(const char *file)
{
    ...
    else
    {
        ...
        for (String::size_type pos = 0U; pos < prefixes.size(); ++pos)
        {
            ...
            pos = std::min(prefixes.find(separator, prefix_start),
                           prefixes.size());

            const auto prefix_size = pos - prefix_start;
            if (prefix_size > longest_match)
            {
                // TODO under DOCTEST_MSVC: does the comparison need strnicmp()
                // to work with drive letter capitalization?
                if (0 == std::strncmp(prefixes.c_str() + prefix_start, file,
                                      prefix_size))
                {
    ...
    else
    {
        if (g_cs->subcaseStack[g_cs->currentSubcaseDepth] == m_signature)
        {
            ...
            DOCTEST_ITERATE_THROUGH_REPORTERS(subcase_start, m_signature);
                 ...
                 g_cs->fullyTraversedSubcases.find(hash(
...
void color_to_stream(std::ostream &s, Color::Enum code)
{
    ...
    static struct ConsoleHelper
    {
        ...
        ConsoleHelper()
        {
            ...
            origFgAttrs = csbiInfo.wAttributes &
                          ~(BACKGROUND_GREEN | BACKGROUND_RED |
                            BACKGROUND_BLUE | BACKGROUND_INTENSITY);
            origBgAttrs = csbiInfo.wAttributes &
...
struct FatalConditionHandler
{
    ...
    {
        // Multiple threads may enter this filter/handler at once. We want the
        // error message to be printed on the console just once no matter how
        // many threads have crashed.
    ...
    FatalConditionHandler()
    {
        ...
        std::set_terminate(
            []() DOCTEST_NOEXCEPT
            {
                ...
                if (isDebuggerActive() && !g_cs->no_breaks)
                    DOCTEST_BREAK_INTO_DEBUGGER();
                std::exit(
                    EXIT_FAILURE); // explicitly exit - otherwise the SIGABRT
        ...
        prev_sigabrt_handler = std::signal(
            SIGABRT,
            [](int signal) DOCTEST_NOEXCEPT
            {
                if (signal == SIGABRT)
                {
                    reportFatal(
        ...
        // offering a choice to debug the aborted program - we want to disable
        // that.
        prev_abort_behavior =
            _set_abort_behavior(0x0, _WRITE_ABORT_MSG | _CALL_REPORTFAULT);
        // In debug mode, the Windows CRT can crash with an assertion over
        // invalid input (e.g. passing an invalid file descriptor). The default
        // handling for these assertions is to pop up a dialog and wait for user
        // input. Instead ask the CRT to dump such assertions to stderr
        // non-interactively.
        prev_report_mode = _CrtSetReportMode(
...
struct XmlReporter : public IReporter
{
    ...
    {
        if (tc) // the TestSuite tag - only if there has been at least 1 test
                // case
            xml.endElement();
    ...
    void test_case_end(const CurrentTestCaseStats &st) override
    {
        xml.startElement("OverallResultsAsserts")
            .writeAttribute("successes", st.numAssertsCurrentTest -
...
struct JUnitReporter : public IReporter
{
    ...
    void test_run_end(const TestRunStats &p) override
    {
        ...
        if (opt.no_time_in_output == false)
        {
            ...
            xml.writeAttribute("timestamp",
                               JUnitTestCaseData::getCurrentTimestamp());
        }
        if (opt.no_version == false)
    ...
    void log_assert(const AssertData &rb) override
    {
        ...
        log_contexts(os);
        testCaseData.addFailure(rb.m_decomp.c_str(), assertString(rb.m_at),
                                os.str());
    }
...
struct ConsoleReporter : public IReporter
{
    ...
    void report_query(const QueryData &in) override
    {
        ...
        else if (opt.list_test_suites)
        {
            ...
            s << Color::Cyan << "[doctest] " << Color::None
              ...
              << g_cs->numTestCasesPassingFilters << "\n";
            s << Color::Cyan << "[doctest] " << Color::None
              << "test suites with unskipped test cases passing the current "
                 "filters: "
    ...
    void test_run_end(const TestRunStats &p) override
    {
        ...
        s << Color::Cyan << "[doctest] " << Color::None
          ...
          << std::setw(failwidth) << p.numAssertsFailed << " failed"
          << Color::None << " |\n";
...
#define DOCTEST_DEBUG_OUTPUT_REPORTER_OVERRIDE(func, type, arg)                \
    ...
    DOCTEST_DEBUG_OUTPUT_REPORTER_OVERRIDE(test_run_start, DOCTEST_EMPTY,
                                           DOCTEST_EMPTY)
    DOCTEST_DEBUG_OUTPUT_REPORTER_OVERRIDE(test_run_end, const TestRunStats &,
                                           in)
                    ...
                    optionType type, int &res)
{
    ...
    {
        // integer
        // TODO: change this to use std::stoi or something else! currently it
...
int Context::run()
{
    ...
    if (!testArray.empty())
    {
        ...
        else if (p->order_by.compare("none", true) == 0)
        {
            ...
            // executable with a specific test case in mind - we don't want to
            // slow down the startup times
        }
    }
    ...
    for (auto &curr : testArray)
    {
        ...
        // execute the test if it passes all the filtering
        {
            ...
            do
            {
                // reset some of the fields for subcases (except for the set of
                // fully passed ones)
                p->reachedLeaf = false;
