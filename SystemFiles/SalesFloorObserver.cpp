...
void SalesFloorObserver::update()
{
    ...
    std::cout << "Sales Floor Update: " << plantTypes.size()
              << " plant types currently available." << std::endl;
    ...
    for (const auto &newItem : availableItems)
    {
        ...
        {
            if (oldItem.first &&
                oldItem.first->getDescription() == plant->getDescription())
            {
        ...
        if (oldQty == 0)
        {
            std::string message = "New stock added: " + description +
        ...
        else if (newQty > oldQty)
        {
            ...
            std::string message = "Stock added: " + std::to_string(added) +
                                  " " + description +
                                  " (Total: " + std::to_string(newQty) + ")";
            notifyStaff(message);
        }
        else if (newQty < oldQty)
        {
            ...
            std::string message =
                "Stock removed: " + std::to_string(removed) + " " +
                description + " (Remaining: " + std::to_string(newQty) + ")";
            notifyStaff(message);
        }
    }
    ...
    for (const auto &oldItem : oldItems)
    {
        ...
        for (const auto &newItem : availableItems)
        {
            if (newItem.first && newItem.first->getDescription() ==
                                     oldItem.first->getDescription())
            {
                found = true;
                break;
            }
        ...
        if (!found)
        {
            ...
            std::string message =
                "Stock depleted: " + description + " is now out of stock";
            notifyStaff(message);
        }
    }
}
...
bool SalesFloorObserver::isPlantAvailable(Plant *plant) const
{
    ...
    {
        if (item.first &&
            item.first->getDescription() == plant->getDescription() &&
            item.second > 0)
        {
            return true;
        }
...
void SalesFloorObserver::displayAvailablePlants() const
{
    ...
    for (const auto &pair : consolidatedItems)
    {
        ...
        std::cout << "   Price: R" << plant->getPrice()
                  << " | Quantity: " << quantity << " | Subtotal: R"
                  << itemValue << "\n";
        std::cout << "----------------------------------------------------\n";
    }
    std::cout << "SUMMARY:\n";
...
{
    if (staff && std::find(notifiedStaff.begin(), notifiedStaff.end(), staff) ==
                     notifiedStaff.end())
    {
...
void SalesFloorObserver::notifyStaff(const std::string &message)
{
    ...
    for (Staff *staff : notifiedStaff)
    {
        if (staff)
        {
            std::cout << "Notifying " << staff->getName() << ": " << message
                      << std::endl;
        }
    }
}
