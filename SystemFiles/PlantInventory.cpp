...
{
    auto it = std::find(inventoryObservers.begin(), inventoryObservers.end(),
                        observer);
...
void PlantInventory::updateStock() { notify(); }
...
int PlantInventory::findPlantIndex(Plant *plant) const
{
    ...
    {
        if (inventoryItems[i].first &&
            inventoryItems[i].first->getDescription() ==
...
const std::vector<std::pair<Plant *, int>>
PlantInventory::getInventoryView() const
{
    return inventoryItems;
}
