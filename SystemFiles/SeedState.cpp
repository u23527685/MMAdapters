...
void SeedState::applyCare(PlantLifeCycle *context, Plant *plant,
...
{
    // Seed considered "healthy" (no immediate care needed) only if it has
    // reached growth threshold
    if (plant->getGrowthProgress() >= 3)
    {
        context->setState(
            std::make_unique<SeedlingState>()); // triggers notify()
        return true;
    }
...
PlantState *SeedState::clone() const { return new SeedState(); }

std::string SeedState::getName() const { return "Seed"; }
