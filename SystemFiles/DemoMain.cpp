...
void simulateDay(std::vector<std::unique_ptr<Plant>>& growingPlants,
    ...
    growingPlants.erase(std::remove_if(growingPlants.begin(),
growingPlants.end(),
        [&died](const auto& p) {
            if (!p || !p->getState()) {
                std::cout << PASTEL_GRAY << "💀 Removing invalid plant (Ptr: "
<< p.get() << ", State: "
                          << (p && p->getState() ? p->getState()->getName() :
"null") << ")\n" << RESET; died++; return true;
            }
            return false;
        }), growingPlants.end());
    if (initialSize != growingPlants.size()) {
        std::cout << PASTEL_YELLOW << "Removed " << (initialSize -
growingPlants.size())
    ...
    if (floorEmployees.empty()) {
        std::cout << PASTEL_SALMON << "⚠️ No Floor Employees available to care
for plants! All plants will be neglected.\n" << RESET;
    }
    ...
    for (auto& plant : growingPlants) {
        if (plant && plant->getState() && (plant->getState()->getName() ==
"Distressed" || plant->getState()->getName() == "Withered")) {
    ...
    for (size_t i = 0; i < allPlants.size(); ++i) {
        ...
        if (!plant || !plant->getState()) {
            std::cout << PASTEL_GRAY << "💀 Plant " << (plant ? plant->getName()
: "Unknown")
...
<< plant->getName()
                      << " (" << plant->getState()->getName() << "). Plant
neglected!\n" << RESET; neglected++; continue;
        }

        PlantLifeCycle cycle(plant,
std::unique_ptr<PlantState>(plant->getState()->clone()), plant->getName());
        cycle.attach(assignedEmployee);

        auto routine = PlantCareRoutine::PlantCare(plant);
        if (!routine) {
            std::cout << PASTEL_SALMON << "❌ No care routine for " <<
plant->getName() << "\n" << RESET; plantsToRemove.push_back(i); died++;
            continue;
        }

        std::cout << PASTEL_GREEN << "📋 [" << assignedEmployee->getName() << "]
noticed " << plant->getName()
                  << " in " << plant->getState()->getName() << " state. Applying
care: "; cycle.getStateObj()->applyCare(&cycle, plant, routine.get());
...
state became null!\n" << RESET; plantsToRemove.push_back(i); died++; continue;
        }
...
has matured. Moving to inventory.\n" << RESET;
            ...
            if (plant->getDescription() == "Red Rose") {
                ...
                std::cout << PASTEL_GREEN << "Created new Red Rose for inventory
(Ptr: " << inventoryPlant << ")\n" << RESET; } else if (plant->getDescription()
== "Yellow Rose") { inventoryPlant = new Rose(9.0, "Yellow Rose");
                inventoryPlant->setCategory("Shade");
                inventoryPlant->setState(std::make_unique<MatureState>());
                std::cout << PASTEL_GREEN << "Created new Yellow Rose for
inventory (Ptr: " << inventoryPlant << ")\n" << RESET; } else if
(plant->getDescription() == "Oak Tree") { inventoryPlant = new Oak(7.0, "Oak
Tree"); inventoryPlant->setCategory("Temperate");
                inventoryPlant->setState(std::make_unique<MatureState>());
                std::cout << PASTEL_GREEN << "Created new Oak Tree for inventory
(Ptr: " << inventoryPlant << ")\n" << RESET;
            }
...
<< RESET; inventory->addStock(inventoryPlant, 1); plantsToRemove.push_back(i);
                matured++;
            } else {
                std::cout << PASTEL_SALMON << "❌ Failed to create new plant for
inventory: "
                          << plant->getDescription() << "\n" << RESET;
            }
        } else if (stateName == "Withered") {
            std::cout << PASTEL_GRAY << "💀 Plant " << plant->getName() << " has
withered and died!\n" << RESET; plantsToRemove.push_back(i); died++;
        }
    }
...
std::greater<size_t>()); for (size_t idx : plantsToRemove) { if (idx <
growingPlants.size()) { growingPlants.erase(growingPlants.begin() + idx);
        }
    }
...
tried to ask a query, but no valid plant found.\n" << RESET; continue;
            ...
            if (queryType == 0) {
                ...
                std::cout << PASTEL_AQUA << customer.getName() << " asking about
care routine for " << randPlant->getDescription() << ".\n" << RESET; } else { q
= queryBuilder.describe(randPlant); std::cout << PASTEL_AQUA <<
customer.getName() << " asking for description of " <<
randPlant->getDescription() << ".\n" << RESET;
            }

            if (q) {
                std::cout << PASTEL_GREEN << "\nHandling query for " <<
randPlant->getDescription() << " by " << chainStart->getName() << "\n" << RESET;
...
<< customer.getName() << ".\n" << RESET;
            ...
            auto inventoryItems = inventory->getInventoryView();
            if (inventoryItems.empty()) {
                std::cout << PASTEL_SALMON << customer.getName() << " wanted to
buy, but no plants available.\n" << RESET; continue;
            }
            auto it = inventoryItems.begin();
            std::advance(it, rand() % inventoryItems.size());
            Plant* buyPlant = it->first;
            if (!buyPlant) {
                std::cout << PASTEL_SALMON << "❌ No valid plant selected for
purchase by " << customer.getName() << ".\n" << RESET; continue;
            }
            int buyQty = rand() % 3 + 1;
...
RESET; buyQty = available;
            }
            if (buyQty == 0) continue;

            std::cout << PASTEL_GREEN << "\nProcessing purchase of " << buyQty
<< " " << buyPlant->getDescription() << " by " << customer.getName() << "\n" <<
RESET;

            Plant* decorated = new BasePlant(buyPlant->getPrice(),
buyPlant->getDescription()); int deco = rand() % 4; if (deco == 0) decorated =
new GiftWrap(decorated); else if (deco == 1) decorated = new
DecorativePot(decorated); else if (deco == 2) decorated = new
SpecialArrangement(decorated);

            double totalPrice = decorated->getPrice() * buyQty;
            Order order(&customer, "ORD-" + std::to_string(rand()));
            Transaction* tx = new Transaction(order.getOrderId(), totalPrice,
buyQty); int payType = rand() % 3; if (payType == 0)
tx->setPaymentStrategy(&creditCard); else if (payType == 1)
tx->setPaymentStrategy(&ewallet); else tx->setPaymentStrategy(&eft);
...
totalPrice << ".\n" << RESET; } else { delete tx; std::cout << PASTEL_SALMON <<
"❌ Purchase failed for " << customer.getName() << ": not enough stock.\n" <<
RESET;
            }
            delete decorated;
        }
...
(Qty: " << item.second
                ...
                std::cout << PASTEL_SALMON << "⚠️ Invalid plant pointer in
inventory!\n" << RESET;
            }
        }
    }
...
RESET; std::cout << PASTEL_MINT << "🌿 Fertilized:    " << fertilized << "\n" <<
RESET; std::cout << PASTEL_PINK << "🌸 Matured:       " << matured << "\n" <<
RESET; std::cout << PASTEL_GRAY << "💀 Died:          " << died << "\n" <<
RESET; std::cout << PASTEL_YELLOW << "⚠️  Neglected:     " << neglected << "\n"
<< RESET; std::cout << PASTEL_ORANGE << "🗓️  Days Passed:   " << day << "\n" <<
RESET; std::cout << "═══════════════════════════════════════════\n"; std::cout
<< PASTEL_GREEN << "🌱 End of day processing complete.\n\n" << RESET;
}
...
std::vector<Transaction*>& savedTransactions, std::map<std::string, int>&
plantMenuOptions, double& balance, CreditCardPaymentStrategy& creditCard,
EWalletPaymentStrategy& ewallet, EFTPaymentStrategy& eft) { std::cout << "\n🌸
Welcome to Plantopia’s Guided Tour! 🌸\n"; std::cout << "I’m Flora the Botanist,
your friendly guide! Let’s explore how to run your magical nursery in 5 fun
steps.\n"; std::cout << "Follow my instructions, and you’ll be a plant pro in no
time! 🌱\n\n"; std::cout << "Type 1 and Enter to start each step, or type 'skip'
...
🌿\n"; return;
    ...
    std::cout << "To grow plants, you need staff! Let’s hire a Floor Employee to
care for your plants.\n"; std::cout << "Your balance is R" << std::fixed <<
std::setprecision(2) << balance << ". Hiring a Floor Employee costs R200.\n\n";
    std::cout << "Type 1 and Enter to hire 'Flora’s Assistant' (Floor Employee),
or type 'skip' to exit.\n"; std::getline(std::cin >> std::ws, input); if (input
== "skip") { std::cout << "Skipping the tutorial! Let’s grow some plants! 🌿\n";
        return;
    }
    ...
    if (balance >= 200) {
        balance -= 200;
        std::cout << "🎉 Hired Flora’s Assistant! They’ll help your plants grow.
Current balance: R" << balance << "\n"; } else { std::cout << "🎉 Hired Flora’s
Assistant for free (tutorial bonus)! They’ll help your plants grow. Current
balance: R" << balance << "\n";
    }

    // Step 2: Buy Seeds
    std::cout << "\n🌱 Step 2: Planting the Seeds of Success 🌱\n";
    std::cout << "Now, let’s buy a Red Rose seed to start your nursery. It costs
R3.00.\n\n"; std::cout << "Type 1 and Enter to buy 1 Red Rose seed, or type
'skip' to exit.\n"; std::getline(std::cin >> std::ws, input); if (input ==
"skip") { std::cout << "Skipping the tutorial! Happy planting! 🌿\n"; return;
    }
...
balance: R" << balance << "\n"; } else { std::cout << "🌹 Bought 1 Red Rose seed
for free (tutorial bonus)! It’s in your nursery. Current balance: R" << balance
<< "\n";
    }
...
simulate a day to see it grow!\n\n"; std::cout << "Type 1 and Enter to simulate
a day, or type 'skip' to exit.\n"; std::getline(std::cin >> std::ws, input); if
(input == "skip") { std::cout << "Skipping the tutorial! Enjoy Plantopia! 🌿\n";
        return;
    }
...
available for sale!\n"; } else { auto matureRose = std::make_unique<Rose>(10.0,
"Red Rose"); matureRose->setCategory("Sunny");
        matureRose->setState(std::make_unique<MatureState>());
        Plant* rawRose = matureRose.release();
        inventory->addStock(rawRose, 1);
...
Rose’s care routine.\n\n"; std::cout << "Type 1 and Enter to ask 'What is the
care routine for Red Rose?', or type 'skip' to exit.\n"; std::getline(std::cin
>> std::ws, input); if (input == "skip") { std::cout << "Skipping the tutorial!
You’re ready for Plantopia! 🌿\n"; return;
    }
    bool hasSalesStaff = false;
    for (auto* staff : hiredStaff) {
        if (dynamic_cast<SalesEmployee*>(staff) ||
dynamic_cast<SalesManager*>(staff)) { hasSalesStaff = true; break;
        }
    }
    if (!hasSalesStaff) {
...
🌟\n";
    ...
    if (selectedPlant) {
        ...
        if (!hiredStaff.empty()) {
            ...
            std::cout << "❌ No staff available to handle query. Imagine Flora’s
Assistant said: 'Water and sun daily!'\n";
        }
        delete query;
    } else {
        std::cout << "❌ No Red Roses to query. Imagine Flora’s Assistant said:
'Water and sun daily!' 😊\n";
    }
...
Gift Wrap decoration.\n"; std::cout << "Type 1 and Enter to sell 1 Red Rose with
Gift Wrap using Credit Card, or type 'skip' to exit.\n"; std::getline(std::cin
>> std::ws, input); if (input == "skip") { std::cout << "Skipping the tutorial!
Happy selling! 🌿\n"; return;
    }
    ...
    for (const auto& item : inventoryItems) {
        if (item.first && item.first->getDescription() == "Red Rose" &&
item.second > 0) { selectedPlant = item.first; break;
        }
    }
    if (selectedPlant) {
        Customer customer("Tutorial Customer");
        Plant* decorated = new BasePlant(selectedPlant->getPrice(),
selectedPlant->getDescription()); decorated = new GiftWrap(decorated); double
total = decorated->getPrice() * 1; if (inventory->removeStock(selectedPlant, 1))
{ Order order(&customer, "ORD-TUTORIAL"); Transaction* tx = new
Transaction(order.getOrderId(), total, 1); tx->setPaymentStrategy(&creditCard);
            order.addTransaction(tx);
            order.processOrder();
            savedTransactions.push_back(tx);
            balance += total;
...
them, and answer customer queries!\n \n"; std::cout << "Type 1 and enter to
start your Plantopia adventure!\n"; std::getline(std::cin >> std::ws, input);
}
...
RESET; std::cout << PASTEL_SALMON << "🌿   Welcome to Plantopia: Terminal
Edition   🌿\n" << RESET; std::cout << PASTEL_MINT <<
"=============================================\n" << RESET;
    ...
    while (true) {
        ...
        if (tutorialChoice == 'y' || tutorialChoice == 'Y' || tutorialChoice ==
'n' || tutorialChoice == 'N') { break;
        }
...
std::make_unique<SalesFloorObserver>(inventory);
    ...
    if (inventoryItems.empty()) {
        ...
        for (const auto& item : inventoryItems) {
            std::cout << "" << item.first->getDescription() << " (Qty: " <<
item.second << ", Ptr: " << item.first << ")\n";
        }
    }
...
RESET;
        ...
        while (true) {
            ...
            if (std::cin.fail()) {
                std::cin.clear();
                std::cin.ignore(std::numeric_limits<std::streamsize>::max(),
'\n'); std::cout << "Invalid input. Please enter a number: "; continue;
            }
            break;
        }
...
std::setprecision(2) << balance; std::cout << "\nChoose staff type:\n1. Floor
Employee (R200)\n2. Floor Manager (R400)\n3. Sales Employee (R300)\n4. Sales
Manager (R500)\n5. Back to staff menu\nEnter number: "; int type; while (true) {
                        std::cin >> type;
...
'\n'); std::cout << "Invalid input. Please enter a number between 1 and 5: ";
                    ...
                    switch (type) {
                        case 1: newStaff = new
FloorEmployee(names[randomIndex]); cost = 200; break; case 2: newStaff = new
FloorManager(names[randomIndex]); cost = 400; break; case 3: newStaff = new
SalesEmployee(names[randomIndex]); cost = 300; break; case 4: newStaff = new
SalesManager(names[randomIndex]); cost = 500; break; case 5: continue;
                    }
...
(" << getStaffRole(newStaff) << ") successfully!\n";
                    }
                }
...
to staff menu\n"; int type; while (true) { std::cin >> type; if (std::cin.fail()
|| type < 1 || type > 4) { std::cin.clear();
                                std::cin.ignore(std::numeric_limits<std::streamsize>::max(),
...
'\n'); std::cout << "Invalid quantity. Please enter a positive number: ";
                        ...
                        double costPerSeed = (type == 1 ? 3.0 : (type == 2
? 2.50 : 1.50)); double totalCost = costPerSeed * qty; if (balance < totalCost)
{ std::cout << "❌ Not enough balance.\n"; continue;
                        ...
                        std::cout << "✅ Purchased " << qty << " seeds and added
to nursery. (for R" << totalCost << ")\n"; balance -= totalCost; std::cout <<
...
🌿\n" << RESET; if (growingPlants.empty()) { std::cout << PASTEL_PINK << "No
plants currently growing.\n" << RESET; } else { for (auto& p : growingPlants) {
                            std::cout << PASTEL_MINT << "- " <<
p->getDescription() << RESET; if (p->getState()) std::cout << " | State: " <<
PASTEL_PURPLE << p->getState()->getName() << RESET; else std::cout << " | State:
" << PASTEL_YELLOW << "None" << RESET; std::cout << " | Category: " <<
PASTEL_ORANGE << p->getCategory() << RESET
                                    << " | Water: " << PASTEL_BLUE <<
p->getCurrentWater() << "/" << p->getMaxWater() << RESET
                                    << " | Sunlight: " << PASTEL_YELLOW <<
p->getCurrentSunlight() << "/" << p->getMaxSunlight() << RESET
                                    << " | Nutrients: " << PASTEL_GREEN <<
p->getCurrentNutrients() << "/" << p->getMaxNutrients() << RESET
                                    << " | Growth: " << PASTEL_PINK <<
...
Sale --- 🌸\n" << RESET; auto inventoryItems = inventory->getInventoryView(); if
(inventoryItems.empty()) { std::cout << PASTEL_PINK << "No plants available for
sale.\n" << RESET; } else { std::map<std::string, std::pair<Plant*, int>>
consolidatedItems; for (const auto& item : inventoryItems) { if (!item.first ||
item.second <= 0) continue; std::string desc = item.first->getDescription(); if
(consolidatedItems.find(desc) == consolidatedItems.end()) {
                                consolidatedItems[desc] = {item.first,
item.second}; } else { consolidatedItems[desc].second += item.second;
                            }
                        }
...
item.second.second << RESET << ")\n";
                    ...
                    std::cout << PASTEL_BLUE << "\n📢 Staff Notifications\n" <<
RESET; std::cout << "Staff are automatically informed of new plants entering and
leaving your nursery.\n\n";

                    std::cout << PASTEL_BLUE << "Currently attached staff:\n" <<
RESET; if (hiredStaff.empty()) { std::cout << PASTEL_SALMON << "⚠️ No staff hired
yet. Hire staff to see them receive notifications!\n\n" << RESET; } else { for
(auto* s : hiredStaff) { std::cout << PASTEL_GREEN << "- " << s->getName() << "
(" << getStaffRole(s) << ")\n" << RESET;
                        }
                        std::cout << "\n";
                    }
...
salesFloor->getNotificationHistory(); if (notifications.empty()) { std::cout <<
PASTEL_SALMON << "📭 No notifications yet.\n" << RESET; std::cout << "Buy seeds,
simulate days, or make sales to see notifications!\n\n"; } else { std::cout <<
PASTEL_BLUE << "📬 Notification History (" << notifications.size() << "
total):\n" << RESET; std::cout <<
"---------------------------------------------------\n";

                        int startIdx = std::max(0,
static_cast<int>(notifications.size()) - 10); for (size_t i = startIdx; i <
notifications.size(); ++i) { std::cout << "[" << (i + 1) << "] "; if
(!hiredStaff.empty()) { for (auto* staff : hiredStaff) { std::cout <<
PASTEL_GREEN << "[Notification to " << staff->getName() << "] " << RESET;
                                }
                            }
...
notifications.size() << " notifications.\n";
                    ...
                    std::cout << PASTEL_GREEN << "\n--- Current Sales Floor
Status ---\n" << RESET; proxy.displayAvailablePlants();
...
'\n'); std::cout << "Invalid input. Please enter a number: "; continue;
                ...
                if (cChoice == 1) {
                    ...
                    if (inventoryItems.empty()) {
                        ...
                        for (const auto& item : inventoryItems) {
                            auto it =
plantMenuOptions.find(item.first->getDescription()); if (it !=
plantMenuOptions.end()) { int qty = item.second; if (qty > 0) std::cout <<
it->second << ". " << item.first->getDescription() << " - R"
                                              << std::fixed <<
std::setprecision(2) << item.first->getPrice()
                                              << " (Qty: " << qty << ")\n";
                                else
                                    std::cout << it->second << ". " <<
item.first->getDescription() << " - ❌ Out of stock!\n";
                            }
                        }
                    }
                }
...
plantMenuOptions.find(item.first->getDescription()); if (it !=
plantMenuOptions.end()) { int qty = item.second; if (qty > 0) { std::cout <<
it->second << ". " << item.first->getDescription()
                                          << " - R" << std::fixed <<
std::setprecision(2) << item.first->getPrice()
                                          << " (Qty: " << qty << ")\n";
                            } else {
...
'\n'); std::cout << "Invalid input. Please enter a number: "; continue;
                    ...
                    for (const auto& item : inventoryItems) {
                        ...
                        auto it =
plantMenuOptions.find(item.first->getDescription()); if (it !=
plantMenuOptions.end() && it->second == choice) { selectedPlant = item.first;
                            break;
                        }
                    }
...
available << ": "; continue;
                    ...
                    Plant* decorated = new BasePlant(selectedPlant->getPrice(),
selectedPlant->getDescription()); bool addingDecor = true; while (addingDecor) {
                        std::cout << "\nAdd decoration?\n"
                                  << "1. Gift Wrap (+R5)\n"
...
'\n'); std::cout << "Invalid input. Please enter a number: "; continue;
                        ...
                        switch (decoChoice) {
                            case 1: decorated = new GiftWrap(decorated);
std::cout << "🎁 Added Gift Wrap\n"; break; case 2: decorated = new
DecorativePot(decorated); std::cout << "🪴 Added Decorative Pot\n"; break; case
3: decorated = new SpecialArrangement(decorated); std::cout << "💐 Added Special
Arrangement\n"; break; default: addingDecor = false; break;
                        }
                    }
...
E-Wallet\n3. EFT\nChoose: "; int pay; while (true) { std::cin >> pay; if
(std::cin.fail() || pay < 1 || pay > 3) { std::cin.clear();
                            std::cin.ignore(std::numeric_limits<std::streamsize>::max(),
'\n'); std::cout << "Invalid input. Please enter a number between 1 and 3: ";
                    ...
                    Order order(&customer, "ORD-" + std::to_string(rand()));
                    Transaction* tx = new Transaction(order.getOrderId(), total,
qty); tx->setPaymentStrategy(strategy); if (proxy.buyPlant(selectedPlant, qty))
{ order.addTransaction(tx); order.processOrder(); balance += total;
                        savedTransactions.push_back(tx);
...
information from last order.\n"; continue;
                    ...
                    int availableQty =
inventory->getQuantity(lastPurchasedPlant); if (availableQty <
lastPurchasedQuantity) { std::cout << "\n❌ Cannot repurchase: Not enough
...
last->getQuantity() << "\n"; std::cout << std::fixed << std::setprecision(2)
            ...
            if (!inventoryItems.empty()) {
                for (const auto& item : inventoryItems) {
                    auto it =
plantMenuOptions.find(item.first->getDescription()); if (it !=
plantMenuOptions.end()) { std::cout << it->second << ". " <<
item.first->getDescription() << " (Qty: " << item.second << ")\n";
                    }
                }
            }
...
'\n');
                ...
                std::string qtype;
                std::cout << PASTEL_SALMON <<"\nEnter query TYPE (FULL CAPS,
e.g. INFO, CARE ROUTINE, STOCK): "; std::getline(std::cin, qtype);
                // normalize to uppercase so user's input becomes full caps
                for (auto &ch : qtype) ch =
...
'\n'); std::cout << "Invalid input. Please enter a number between 1 and 4: ";
            ...
            for (auto* staff : hiredStaff) {
                if (dynamic_cast<FloorEmployee*>(staff) && (queryType == "INFO"
|| queryType == "CARE ROUTINE")) { canHandle = true; break;
                }
                if (dynamic_cast<FloorManager*>(staff) && queryType == "STOCK")
{ canHandle = true; break;
                }
                if ((dynamic_cast<SalesEmployee*>(staff) ||
dynamic_cast<SalesManager*>(staff)) && queryType == "STOCK") { canHandle = true;
                    break;
                }
            }

            if (!canHandle) {
                std::cout << "❌ No " << requiredStaffType << " hired to handle
this query. Please hire the appropriate staff.\n"; delete query; continue;
            }
            ...
            delete query;
                ...
                // Try to Steal Plants (Proxy Demo)
                    ...
                    std::cout << "\n" << PASTEL_MINT <<
"╔════════════════════════════════════════════╗\n"; std::cout << "    🦹
ATTEMPTING TO STEAL PLANTS\n"; std::cout <<
"╚════════════════════════════════════════════╝\n" << RESET;

                    std::cout << "\n" << PASTEL_YELLOW << "⚠️  Shady Customer:
\"Let me try to mess with the inventory...\"\n" << RESET;

                    // Show available plants first
                    auto inventoryItems = inventory->getInventoryView();
                    if (inventoryItems.empty()) {
                        std::cout << "\n" << PASTEL_GRAY << "No plants in
inventory to steal. Come back later!\n" << RESET; continue;
                    }

                    std::cout << "\n" << PASTEL_CYAN << "Available plants in
inventory:\n" << RESET; for (const auto& item : inventoryItems) { auto it =
plantMenuOptions.find(item.first->getDescription()); if (it !=
plantMenuOptions.end()) { std::cout << "  " << it->second << ". " <<
item.first->getDescription()
                    ...
                    std::cout << PASTEL_SALMON << "1. Try to add unauthorized
plants to inventory\n" << RESET; std::cout << PASTEL_SALMON << "2. Try to steal
plants from inventory\n" << RESET; std::cout << PASTEL_GRAY << "3. Cancel\n" <<
RESET; std::cout << PASTEL_MINT << "Choice: " << RESET;

                    int stealChoice;
                    while (true) {
                        std::cin >> stealChoice;
                        if (std::cin.fail() || stealChoice < 1 || stealChoice >
3) { std::cin.clear();
                            std::cin.ignore(std::numeric_limits<std::streamsize>::max(),
'\n'); std::cout << "Invalid input. Please enter 1-3: "; continue;
                        }
                        break;
                    }
...
'\n'); std::cout << "Invalid input. Please enter a number: "; continue;
                    ...
                    for (const auto& item : inventoryItems) {
                        auto it =
plantMenuOptions.find(item.first->getDescription()); if (it !=
plantMenuOptions.end() && it->second == plantChoice) { selectedPlant =
item.first; break;
                        }
                    }

                    if (!selectedPlant) {
                        std::cout << PASTEL_SALMON << "❌ Invalid plant
selection!\n" << RESET; continue;
                    }
...
'\n'); std::cout << "Invalid quantity. Please enter a positive number: ";
                    ...
                    std::cout << "\n" << PASTEL_YELLOW <<
"═══════════════════════════════════════════\n" << RESET; std::cout <<
PASTEL_PINK << "        🛡️  PROTECTION IN ACTION 🛡️\n" << RESET; std::cout <<
PASTEL_YELLOW << "═══════════════════════════════════════════\n" << RESET;

                    if (stealChoice == 1) {
                        std::cout << "\n" << PASTEL_SALMON << "Customer
attempting to add unauthorized ("
                                  << selectedPlant->getDescription() << ", " <<
qty << ")\n" << RESET; std::cout << PASTEL_CYAN << "   Response:\n" << RESET;
                        std::cout << "   → ";
                        proxy.addStock(selectedPlant, qty);

                        std::cout << "\n" << PASTEL_SALMON << "🚨 ALERT! ALERT!
Unauthorized access detected! 🚨\n" << RESET; std::cout << PASTEL_BLUE << "👮
Police: \"FREEZE! You're under arrest for attempting inventory fraud!\"\n" <<
RESET; std::cout << PASTEL_YELLOW << "🦹 Customer: \"Wait, I was just
testing...\"\n" << RESET; std::cout << PASTEL_BLUE << "👮 Police: \"Tell it to
the judge! You have the right to remain silent...\"\n" << RESET; std::cout <<
PASTEL_SALMON << "🚔 *HANDCUFFS CLICK* 🚔\n" << RESET;
...
qty << ")\n" << RESET; std::cout << PASTEL_CYAN << "   Response:\n" << RESET;
                        ...
                        std::cout << "\n" << PASTEL_SALMON << "🚨 SECURITY
BREACH! Theft attempt detected! 🚨\n" << RESET; std::cout << PASTEL_BLUE << "👮
Police: \"STOP RIGHT THERE! Hands where I can see them!\"\n" << RESET; std::cout
<< PASTEL_YELLOW << "🦹 Customer: \"It was just a prank bro!\"\n" << RESET;
                        std::cout << PASTEL_BLUE << "👮 Police: \"Pranks are for
YouTube, not our nursery! You're coming with us!\"\n" << RESET; std::cout <<
PASTEL_SALMON << "🚓 *WEE-OOO WEE-OOO* Police car arrives 🚓\n" << RESET;

                    }

                    std::cout << "\n" << PASTEL_GRAY << "Press Enter to
continue..." << RESET;
                    std::cin.ignore(std::numeric_limits<std::streamsize>::max(),
'\n'); std::cin.get();
                }
...
/* Jerusha - I used your code as well as asked chat to do some validation i
still need to simulate the plant part of it and how it gets cared by the
employees also the code looks good Thank you so much!!! I did not want to mess
or change something so its commented but i did use it above*/
...
//                 if (money < cost) {
//                     std::cout << "Not enough money! (" << money << ") Cost:
//                     $" << cost << "\n"; break;
//                 }
//                 money -= cost;
//                 inventory->addStock(plant, qty);
//                 std::cout << "Bought " << qty << " " <<
//                 plant->getDescription() << "(s).\n";

//                 if (lifecycles.find(plant) == lifecycles.end()) {
//                     PlantLifeCycle* cycle = new PlantLifeCycle(plant, new
//                     SeedState(), plant->getDescription() + " Cycle");
...
//                 Staff* newStaff = nullptr;
//                 double hireCost = 0.0;
//                 std::string name = "Staff" + std::to_string(hiredStaff.size()
//                 + 1); switch (staffChoice) {
//                     case 1:
//                         if (floorEmployee) {
...
//                     case 3:
//                         if (salesEmployee) {
//                             std::cout << "Already hired a Sales Employee.
//                             ("<< name << "\n"; continue;
//                         }
...
//                     salesEmployee->setNext(salesManager ?
//                     static_cast<Staff*>(salesManager) : nullptr);
//                 }
...
//                     p->setCurrentWater(std::max(0.0, p->getCurrentWater() -
//                     decay)); p->setCurrentSunlight(std::max(0.0,
//                     p->getCurrentSunlight() - decay));
//                     p->setCurrentNutrients(std::max(0.0,
//                     p->getCurrentNutrients() - decay)); cycle->updatePlant();
//                 }

//                 int numCustomers = rand() % 5 + 1;
//                 std::cout << numCustomers << " customers visited today.\n";
//                 Staff* chainStart = floorEmployee ?
//                 static_cast<Staff*>(floorEmployee) :
//                     floorManager ? static_cast<Staff*>(floorManager) :
//                     salesEmployee ? static_cast<Staff*>(salesEmployee) :
//                     salesManager ? static_cast<Staff*>(salesManager) :
//                     nullptr;
...
//                     if (action == 0) {
//                         if (!chainStart) {
//                             std::cout << customer.getName() << " asked a
//                             query but no staff available.\n"; continue;
//                         }
...
//                                 std::cout << customer.getName() << " asking
//                                 about care routine.\n";
//                             } else if (queryType == 1) {
...
//                         } else {
//                             q = askQuery.stockInfo();
//                             std::cout << customer.getName() << " asking about
//                             stock info.\n";
//                         }
//                         chainStart->handleQuery(q);
//                         delete q;
...
//                         if (lifecycles.empty()) {
//                             std::cout << customer.getName() << " wanted to
//                             buy but no plants available.\n"; continue;
//                         }
...
//                             std::cout << customer.getName() << " didn't buy "
//                             << buyPlant->getDescription() << " because it's "
//                             << state << ".\n"; continue;
//                         }
//                         int buyQty = rand() % 3 + 1;
//                         int available =
//                         salesFloor->getPlantQuantity(buyPlant); if (available
//                         < buyQty) {
//                             std::cout << customer.getName() << " wanted " <<
//                             buyQty << " but only " << available << "
//                             available.\n"; buyQty = available;
//                         }
//                         if (buyQty == 0) continue;

//                         Plant* decorated = new
//                         BasePlant(buyPlant->getPrice(),
//                         buyPlant->getDescription()); int deco = rand() % 4;
//                         if (deco == 0) {
...
//                         std::to_string(rand())); Transaction* tx = new
//                         Transaction(order.getOrderId(), totalPrice, buyQty);
//                         int payType = rand() % 3;
//                         if (payType == 0)
//                         tx->setPaymentStrategy(&creditCard); else if (payType
//                         == 1) tx->setPaymentStrategy(&ewallet); else
...
//                         money += totalPrice;
//                         std::cout << customer.getName() << " bought " <<
//                         buyQty << " " << decorated->getDescription() << " for
//                         $" << totalPrice << ".\n";
...
//             }
//             case 4: {
//                 std::cout << "\nCurrent Money: $" << std::fixed <<
//                 std::setprecision(2) << money << "\n"; std::cout << "Hired
//                 Staff:\n"; for (auto staff : hiredStaff) {
...
//                     std::cout << "  Water: " << p->getCurrentWater() << "/"
//                     << p->getMaxWater() << "\n"; std::cout << "  Sunlight: "
//                     << p->getCurrentSunlight() << "/" << p->getMaxSunlight()
//                     << "\n"; std::cout << "  Nutrients: " <<
//                     p->getCurrentNutrients() << "/" << p->getMaxNutrients()
//                     << "\n";
//                 }
//                 break;
//             }
