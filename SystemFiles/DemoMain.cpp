...
== "Yellow Rose")
                {
                    ...
                    std::cout << PASTEL_GREEN
                              << "Created new Yellow Rose for
                        inventory(Ptr
                                  : " << inventoryPlant << ")\n
                        " << RESET; } else if
                        (plant->getDescription() == "Oak Tree")
                    { inventoryPlant = new Oak(7.0, "Oak
...
(Ptr: " << inventoryPlant << ")\n" << RESET;
                    ...
                    else
                    {
                        std::cout << PASTEL_SALMON
                                  << "❌ Failed to create new plant for
                            inventory : "
                ...
                if (action == 0)
                {
                    if (!chainStart)
                    {
                        std::cout << PASTEL_SALMON << "❌ "
                                  << customer.getName()
                                  << "
                            tried to ask a query,
                            but no staff available.\n " << RESET; continue;
                    }
...
= queryBuilder.describe(randPlant);
                ...
                else
                {
                    ...
                    if (available < buyQty)
                    {
                        std::cout << PASTEL_YELLOW << customer.getName()
                                  << " wanted " << buyQty << " "
                                  << buyPlant->getDescription() << ", but only "
                                  << available << " available.\n"
                                  << RESET;
                        buyQty = available;
                    }
                    ...
                    Plant *decorated = new BasePlant(
                        buyPlant->getPrice(), buyPlant->getDescription());
                    int deco = rand() % 4;
                    if (deco == 0)
                    ...
                    if (inventory->removeStock(buyPlant, buyQty))
                    {
                        ...
                        std::cout << PASTEL_GREEN << customer.getName()
                                  << " bought " << buyQty << " "
...
(Qty: " << item.second
            ...
            EWalletPaymentStrategy &ewallet, EFTPaymentStrategy &eft)
        {
            std::cout << "\n🌸
                ...
                your friendly guide !Let’s explore how to run your magical
                    nursery in 5 fun steps.\n
                "; std::cout << " Follow my instructions,
                and you’ll be a plant pro in no time ! 🌱\n\n
                    "; std::cout << " Type 1 and
                    Enter to start each step,
...
>> std::ws, input);
    if (input == "skip")
    { std::cout << "Skipping the tutorial!
...
🎉\n "; std::cout << " You’re now ready to run your nursery.Grow plants,
        sell them,
        and answer customer queries !\n \n "; std::cout << " Type 1 and
            enter to start your Plantopia adventure !\n
            "; std::getline(std::cin >> std::ws, input);
        }
        ...
        int main()
        {
            ...
            if (tutorialChoice == 'y' || tutorialChoice == 'Y')
            {
                runTutorial(growingPlants, inventory, hiredStaff,
                            savedTransactions, plantMenuOptions, balance,
                ...
                if (roleChoice == 1)
                {
                    ...
                    while (staffRunning)
                    {
                        ...
                        std::cout << PASTEL_CYAN
                                  << "4. View Nursery Inventory\n"
                                  << RESET;
                        std::cout << PASTEL_AQUA
                                  << "5. View staff notifications\n"
                                  << RESET;
...
🌿\n " << RESET; if (growingPlants.empty()) { std::cout << PASTEL_PINK << " No
                           ...
                           std::cout
                        << PASTEL_MINT << "- " << p->getDescription() << RESET;
                    if (p->getState())
                       ...
                       (inventoryItems.empty())
            {
                ...
                for (const auto &item : consolidatedItems)
                {
                    ...
                    {
                        std::cout
                            << PASTEL_GREEN << it->second << ". " << item.first
                            << RESET << " - R" << PASTEL_YELLOW << std::fixed
                            << std::setprecision(2)
                            << item.second.first->getPrice() << RESET
                            << " (Qty: " << PASTEL_PURPLE << item.second.second
                            << RESET << ")\n";
                    }
                }
            }
        ...
        else if (sChoice == 5)
        {
            std::cout << PASTEL_BLUE << "\n📢 Staff Notifications\n" << RESET;
            std::cout
                << "Staff are automatically informed of new plants entering and
                   leaving your nursery.\n\n ";

                   std::cout
                << PASTEL_BLUE << "Currently attached staff:\n"
                << RESET;
            if (hiredStaff.empty())
            {
                std::cout
                    ...
                    << "⚠️ No staff hired
                       ...
                       " << RESET; } else { for
                       (auto *s
                        : hiredStaff){std::cout << PASTEL_GREEN << "- "
                                                << s->getName()
                                                << "
                                      (" << getStaffRole(s) << ")\n " << RESET;
                       } std::cout
...
"---------------------------------------------------\n";
                ...
                int startIdx =
                    std::max(0, static_cast<int>(notifications.size()) - 10);
                for (size_t i = startIdx; i < notifications.size(); ++i)
                {
                    ...
                    if (!hiredStaff.empty())
                    {
                        for (auto *staff : hiredStaff)
                        {
                            std::cout << PASTEL_GREEN << "[Notification to "
                                      << staff->getName() << "] " << RESET;
                        }
                    }
                ...
                {
                    std::cout << "Showing last 10 of " << notifications.size()
                              << " notifications.\n";
                }
            }
...
else if (roleChoice == 2)
{
    ...
    while (custRunning)
    {
        ...
        else if (cChoice == 2)
        {
            ...
            std::cout << "\nSelect payment method:\n1. Credit Card\n2.
                             E -
                             Wallet\n3.EFT\nChoose
                : "; int pay; while (true) { std::cin >> pay; if
                  (std::cin.fail() || pay < 1 || pay > 3)
            {
                std::cin.clear();
                std::cin.ignore(std::numeric_limits<std::streamsize>::max(),
                                '\n');
        ...
        if (pay == 1)
            ...
            std::cout << "EFT\n";
        std::cout << "-----------------------------------------\n";
        std::cout << std::fixed << std::setprecision(2) << "Total: R" << total
                  << "\n";
        std::cout << "=========================================\n";
    ...
    else if (cChoice == 3)
    {
        ...
        if (proxy.buyPlant(lastPurchasedPlant, lastPurchasedQuantity))
        {
            ...
            std::cout << "-----------------------------------------\n";
            std::cout << std::fixed << std::setprecision(2) << "Total: R"
                      << last->getAmount() << "\n";
            std::cout << "=========================================\n";
    ...
    else if (cChoice == 4)
    {
        ...
        std::cout << std::fixed << std::setprecision(2) << "Total Paid: R"
                  << last->getAmount() << "\n";
        std::cout << "=========================================\n";
...
static_cast<char>(std::toupper((unsigned char)ch));
        ...
        for (auto *staff : hiredStaff)
        {
            if (dynamic_cast<FloorEmployee *>(staff) &&
                (queryType == "INFO" || queryType == "CARE ROUTINE"))
            {
                canHandle = true;
                break;
            }
...
\"Let me try to mess with the inventory...\"\n"
    ...
    std::cout << PASTEL_SALMON
              << "1. Try to add unauthorized
        plants to inventory\n
        " << RESET; std::cout << PASTEL_SALMON << " 2. Try to steal plants from
