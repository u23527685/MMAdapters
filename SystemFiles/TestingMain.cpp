...
int main()
{
    ...
    // Create sample plants (raw pointers; inventory takes ownership per project
    // conventions)
    ...
    order.processOrder();
    std::cout << "Processed order " << order.getOrderId()
    ...
    // Staff and queries: set up a small chain and a query for an inventory
    // plant
    ...
    {
        if (it.first)
            std::cout << "Final Inventory: " << it.first->getDescription()
                      << " qty=" << it.second << "\n";
    }
