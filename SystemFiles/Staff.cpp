...
void Staff::setNext(Staff *staff) { next = staff; }

Staff *Staff::getNext() { return next; }
...
void Staff::getStock(Plant *plant)
{
    ...
    {
        if (item.first && item.second > 0 && item.first == plant)
        { // Check for valid Plant* and quantity
            int quantity = item.second;
            double itemValue = item.first->getPrice() * quantity;
            totalCount += quantity;
            totalValue += itemValue;
            std::cout << item.first->getDescription() << "\n";
            std::cout << "   Price: R" << std::fixed << std::setprecision(2)
                      << item.first->getPrice() << " | Quantity: " << quantity
                      << " | Subtotal: R" << itemValue << "\n";
            std::cout
                << "----------------------------------------------------\n";
        }
    }
    ...
    std::cout << "   Inventory Value: R" << std::fixed << std::setprecision(2)
              << totalValue << "\n";
    std::cout << "====================================================\n";
}
...
void Staff::update(PlantLifeCycle* p){
    ...
    std::cout<< "[Staff] " << name << " noticed " << p->getName() << " is now in
state: " << state << std::endl; if(!p->isHealthy()){ careRoutine =
careRoutine->PlantCare(plant); std::cout << "[Staff] " << name; if(state ==
"Withered" && p->getCurrentWater() < 10  ){ careRoutine->Watering(plant); } else
if(state == "Withered" &&p->getCurrentSunlight() < 10){
...
//     std::cout << "[Staff] " << name << " noticed " << p->getName() << " is
now in state: " << stateName << std::endl;

//     careRoutine = careRoutine->PlantCare(plant);

//    if (!p->isHealthy()) {
//         std::cout << "[Staff] " << name << " is applying care..." <<
std::endl;
...
//     }
    ...
    std::cout << "[Staff] " << name << " noticed " << p->getName() << " is now
...
void Staff::isHealthy(PlantLifeCycle *p)
{
    ...
    std::cout << "[Staff] " << name << " noticed " << p->getName()
              << " is now in state: " << stateName << std::endl;

    std::unique_ptr<PlantCareRoutine> careRoutine =
